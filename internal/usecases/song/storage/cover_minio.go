...
type MinIOCoverStorage struct {
	client        *minio.Client
	bucket        string
	publicBaseURL string
}

func NewMinIOCoverStorage(client *minio.Client, bucket string, publicBaseURL string) *MinIOCoverStorage {
	return &MinIOCoverStorage{
		client:        client,
		bucket:        bucket,
...
func (s *MinIOCoverStorage) EnsureBucketPublic(ctx context.Context) error {
...
func (s *MinIOCoverStorage) UploadCover(ctx context.Context, filter UploadCoverFilter) (string, error) {
	src, err := filter.CoverFile.Open()
	if err != nil {
		return "", fmt.Errorf("open cover file: %v", err)
	}
	defer src.Close()

	avatarBytes, err := io.ReadAll(io.LimitReader(src, maxAvatarSizeBytes+1))
	if err != nil {
		return "", fmt.Errorf("read cover file: %v", err)
	}
	...
	objectName, err := buildAvatarObjectName(filter.SongID, extension)
	if err != nil {
		return "", fmt.Errorf("build cover object name: %v", err)
	}
...
func detectAvatarFormat(fileBytes []byte) (string, string, error) {
	if _, _, err := image.DecodeConfig(bytes.NewReader(fileBytes)); err != nil {
		return "", "", fmt.Errorf("decode cover format: %w", ErrInvalidCoverType)
	}
